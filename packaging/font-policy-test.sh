...
require_line() {
    ...
    line=$2
    reason=$3
    if ! grep -F -x "$line" "$path" >/dev/null; then
        echo "FAIL: $reason" >&2
        exit 1
    fi
}
...
require_line "$tmp_dir/canonical/packaging/debian/control" \
    '               fonts-dejavu-core,' \
    'Debian package tests do not declare the DejaVu fallback font'
require_line "$tmp_dir/canonical/packaging/fedora/realm.spec" \
    'BuildRequires:  dejavu-sans-fonts' \
    'Fedora package tests do not declare DejaVu Sans'
require_line "$tmp_dir/canonical/packaging/fedora/realm.spec" \
