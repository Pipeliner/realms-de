...
EOF
  ...
  session-boots = pkgs.testers.nixosTest {
    ...
    testScript =
      ...
      in
      ''
      # Everything asserted here is true of realm 0.1.0 today.
      ...
      # realm-bar and realm-wm do not exist yet, so this block does not run.
      ...
      if pending_m2:
          ...
          # Under river 0.4 the compositor is inert until realm-wm attaches as
          # its window manager, so this is the liveness check that matters.
          machine.wait_until_succeeds("pgrep -u alice realm-wm")
          machine.wait_until_succeeds("pgrep -u alice realm-bar")
