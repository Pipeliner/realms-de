...
inventory=$(awk -v image="$image" '
...

    {
        ...
        if (job == "fedora-rpm-package") {
            ...
            if (line == "          packaging/tool-sources/build-native-source-kits.sh \"$RUNNER_TEMP/realm-native-kits\"") {
                rpm_source_kit_producers++
            }
            if (line == "          cp \"$RUNNER_TEMP/realm-native-kits/realm-0.1.0.tar.gz\" \"$RUNNER_TEMP/rpmbuild/SOURCES/\"") {
                rpm_source0_copies++
            }
            if (line == "          cp \"$RUNNER_TEMP/realm-native-kits/realm.spec\" \"$RUNNER_TEMP/rpmbuild/SPECS/\"") {
                rpm_spec_copies++
            }
            if (line == "          rpmbuild -bb --nodeps \\") {
                rpmbuild_invocations++
            }
            if (line == "            \"$RUNNER_TEMP/rpmbuild/SPECS/realm.spec\"") {
...
rpm="$root/packaging/fedora/realm.spec"
assert_contains packaging/fedora/realm.spec '# realm — Fedora 44 pre-alpha spec (ADR 0015 / SPEC 0009).' \
...
realm_river_metadata_count=$(printf '%s\n' "$rpm_dependency_metadata" | grep -F -c -e 'realm-river' || true)
if ! { [ "$river_metadata_count" -eq 1 ] && [ "$canonical_river_count" -eq 1 ] &&
    [ "$realm_river_metadata_count" -eq 0 ]; }; then
    fail 'RPM must contain only the canonical active river dependency'
fi
assert_contains packaging/fedora/realm.spec \
