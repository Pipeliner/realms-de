...
pub enum ReadOnce {
    /// One recvmsg admitted these resources.
    Read {
        /// Bytes admitted by the recvmsg.
        bytes: usize,
        /// File descriptors admitted by the recvmsg.
        fds: usize,
    },
    /// The nonblocking socket had no input.
    WouldBlock,
}

/// Outcome of attempting one already-admitted protocol message.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum DispatchOne {
    /// Exactly one protocol message was handled.
    Dispatched,
    /// The incremental parser needs another ingress attempt.
    NeedRead,
    /// Dispatch was refused because an exclusive prepared read is live.
    PreparedReadLive,
}

/// Outcome of at most one nonblocking sendmsg attempt.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum FlushOnce {
    /// No buffered output remains.
    Complete,
    /// Bytes were written, but buffered output remains.
    Progress {
        /// Bytes written by the single sendmsg.
        bytes: usize,
    },
    /// The socket would block; no retry was made.
    WouldBlock,
}
...
impl Backend {
    ...
    /// Attempt at most one nonblocking sendmsg of buffered output.
    #[cfg(not(feature = "client_system"))]
    pub fn flush_once(&self) -> Result<FlushOnce, WaylandError> {
        self.backend.flush_once()
    }
    ...
    /// Exclusively prepare one bounded socket ingress operation.
    #[cfg(not(feature = "client_system"))]
    #[must_use]
    pub fn prepare_read_bounded(&self) -> Option<BoundedReadEventsGuard> {
        client_impl::InnerReadEventsGuard::try_new_bounded(self.backend.clone())
            .map(|guard| BoundedReadEventsGuard { guard })
    }

    /// Parse and handle at most one already-admitted protocol message.
    #[cfg(not(feature = "client_system"))]
    pub fn dispatch_one_pending(&self) -> Result<DispatchOne, WaylandError> {
        self.backend.dispatch_one_pending()
    }
...
/// Exclusive prepared read owned across one poll operation.
#[cfg(not(feature = "client_system"))]
#[derive(Debug)]
pub struct BoundedReadEventsGuard {
    guard: client_impl::InnerReadEventsGuard,
}

#[cfg(not(feature = "client_system"))]
impl BoundedReadEventsGuard {
    /// Access the Wayland socket FD for polling.
    pub fn connection_fd(&self) -> BorrowedFd<'_> {
        self.guard.connection_fd()
    }

    /// Consume the guard and perform exactly one nonblocking recvmsg attempt.
    pub fn read_once(self) -> Result<ReadOnce, WaylandError> {
        self.guard.read_once()
    }
}
