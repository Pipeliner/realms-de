...
/// Maximum bytes admitted by one Realm ingress operation.
pub const MAX_BYTES_IN_ONCE: usize = 16_384;
/// Maximum received FDs retained by one Realm connection.
pub const MAX_FDS_IN: usize = 253;
/// Maximum partial-frame bytes retained by one Realm connection.
pub const MAX_RETAINED_BYTES_IN: usize = 65_535;

#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum SocketFlushOnce {
    Complete,
    Progress(usize),
    WouldBlock,
}
...
impl Socket {
    ...
    /// Realm opt-in: receive once with fixed ancillary capacity and truncation checks.
    pub fn rcv_msg_bounded(
        &self,
        buffer: &mut [u8],
            ...
            .intersects(ReturnFlags::TRUNC | ReturnFlags::CTRUNC)
        {
            return Err(std::io::Error::new(
                ...
                "truncated Wayland recvmsg",
            ));
        }
        let fd_count = received_fds.len();
        fds.extend(received_fds);
        #[cfg(any(target_os = "macos", target_os = "redox"))]
        for fd in fds.iter() {
            if let Ok(flags) = rustix::io::fcntl_getfd(fd) {
...
impl BufferedSocket {
    ...
    /// Attempt at most one nonblocking send of buffered output.
    pub fn flush_once(&mut self) -> IoResult<SocketFlushOnce> {
        let bytes = self.out_data.get_contents();
        if bytes.is_empty() {
            return Ok(SocketFlushOnce::Complete);
        }
        let mut bytes_to_write = bytes;
        let mut fds_to_write = self.out_fds.as_slice();
        if fds_to_write.len() > MAX_FDS_OUT {
            bytes_to_write = &bytes_to_write[..1];
            fds_to_write = &fds_to_write[..MAX_FDS_OUT];
        }
        ...
        match self.socket.send_msg(bytes_to_write, fds_to_write) {
            Ok(count) => {
                ...
                self.out_data.move_to_front();
                self.out_fds.drain(..fds_to_write.len());
                if self.out_data.get_contents().is_empty() {
                    Ok(SocketFlushOnce::Complete)
                } else {
                    Ok(SocketFlushOnce::Progress(count))
                }
            }
            Err(error) if error.kind() == ErrorKind::WouldBlock => Ok(SocketFlushOnce::WouldBlock),
            Err(error) => Err(error),
        }
    }
        ...
        loop {
            match write_to_buffers(msg, self.out_data.get_writable_storage(), &mut self.out_fds) {
                ...
                Err(MessageWriteError::BufferTooSmall) => {
                    ...
                    if self
                        .max_buffer_size
                        .is_some_and(|s| s <= self.out_data.storage.len())
                    {
                        return Ok(false);
                    }
    ...
    /// Admit at most one fixed-size recvmsg into bounded retained storage.
    pub fn fill_incoming_buffers_once(&mut self) -> IoResult<(usize, usize)> {
        // reorganize the buffers
        self.in_data.move_to_front();
        self.in_data.ensure_capacity(MAX_RETAINED_BYTES_IN);
        if self.in_data.get_contents().len() >= MAX_RETAINED_BYTES_IN {
            self.discard_incoming();
            return Err(std::io::Error::new(
                std::io::ErrorKind::InvalidData,
                "Wayland retained-byte limit exceeded",
            ));
        }
        // receive a message
        let received = {
        ...
        if self.in_fds.len() > MAX_FDS_IN {
            self.discard_incoming();
            return Err(std::io::Error::new(
                std::io::ErrorKind::InvalidData,
                "Wayland retained-FD limit exceeded",
            ));
        }
        if in_bytes == 0 {
            // the other end of the socket was closed
...
impl<T: Copy + Default> Buffer<T> {
    ...
    fn ensure_capacity(&mut self, capacity: usize) {
        if self.storage.len() < capacity {
            self.storage.resize_with(capacity, Default::default);
        }
    }
        ...
        let msg = Message {
            ...
            args: smallvec![
                ...
                Argument::Str(Some(Box::new(
                    CString::new(&b"I like trains!"[..]).unwrap()
                ))),
