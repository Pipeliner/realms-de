...
use super::{
    ...
    socket::{BufferedSocket, Socket, SocketFlushOnce},
...
impl InnerBackend {
    ...
    pub fn flush_once(&self) -> Result<FlushOnce, WaylandError> {
        let mut guard = self.state.lock_protocol();
        guard.no_last_error()?;
        match guard.socket.flush_once() {
            Ok(SocketFlushOnce::Complete) => Ok(FlushOnce::Complete),
            Ok(SocketFlushOnce::Progress(bytes)) => Ok(FlushOnce::Progress { bytes }),
            Ok(SocketFlushOnce::WouldBlock) => Ok(FlushOnce::WouldBlock),
            Err(error) => Err(guard.store_and_return_error(error)),
        }
    }
...
impl InnerReadEventsGuard {
    ...
    pub fn read(mut self) -> Result<usize, WaylandError> {
        ...
        if guard.prepared_reads == 0 {
            ...
            let _guard = condvar
                .wait_while(guard, |backend| serial == backend.read_serial)
                .unwrap();
        ...
        let message_desc = match object.interface.requests.get(opcode as usize) {
            ...
            None => {
                panic!(
                    ...
                    opcode, object.interface.name, id.id
                );
            }
        };
...
fn dispatch_events(state: Arc<ConnectionState>) -> Result<usize, WaylandError> {
    dispatch_events_inner(state, false).map(|(dispatched, _)| dispatched)
}

fn dispatch_events_inner(
    state: Arc<ConnectionState>,
    bounded: bool,
) -> Result<(usize, Option<DispatchOne>), WaylandError> {
    let backend = Backend {
        backend: InnerBackend { state },
    };
    ...
    loop {
        ...
        let message = match socket.read_one_message(|id, opcode| {
            ...
            Err(MessageParseError::MissingData) | Err(MessageParseError::MissingFD) => {
                if bounded {
                    drop(guard);
                    backend.backend.state.lock_read().bounded_needs_read = true;
                    return Ok((dispatched, Some(DispatchOne::NeedRead)));
                }
        ...
        let message_desc = receiver
            .interface
            .events
            .get(message.opcode as usize)
            .unwrap();
        ...
        if message.sender_id == 1 {
            ...
            if bounded {
                return Ok((dispatched, Some(DispatchOne::Dispatched)));
            }
            continue;
        }
