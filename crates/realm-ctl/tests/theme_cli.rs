...
use realmctl::run_from;

fn shipped_palette() -> Palette {
    Palette::from_toml(realm_theme::SHIPPED_PALETTE).expect("shipped palette must parse")
}
...
fn realmctl<I, S>(args: I) -> Output
where
    I: IntoIterator<Item = S>,
    S: AsRef<std::ffi::OsStr>,
{
    Command::new(env!("CARGO_BIN_EXE_realmctl"))
        .args(args)
        .output()
        .expect("realmctl runs")
}

fn realmctl_without_config_env<I, S>(args: I) -> Output
where
    I: IntoIterator<Item = S>,
    S: AsRef<std::ffi::OsStr>,
{
    Command::new(env!("CARGO_BIN_EXE_realmctl"))
        ...
        .args(args)
        .output()
        .expect("realmctl runs without configuration environment")
}
...
fn realmctl_at<I, S>(root: &Path, args: I) -> Output
where
    I: IntoIterator<Item = S>,
    S: AsRef<std::ffi::OsStr>,
{
...
fn edit_palette(root: &Path) {
    let path = root.join("realm/palette.toml");
...
fn lint_explicit_palette_needs_no_home_or_xdg_config_home() {
    ...
    let out =
        realmctl_without_config_env(["theme", "lint", "--palette", palette.to_str().unwrap()]);
...
#[cfg(unix)]
#[test]
fn lint_refuses_a_symlinked_realm_directory_without_touching_its_destination() {
    use std::os::unix::fs::symlink;

    let temp = tempfile::tempdir().expect("temporary config root");
    let root = temp.path().join("config");
    let outside = temp.path().join("outside");
    std::fs::create_dir_all(outside.join("realm")).expect("create outside realm directory");
    let bad_palette = write_bad_palette(&outside.join("realm"));
    std::fs::rename(bad_palette, outside.join("realm/palette.toml")).expect("name outside palette");
    std::fs::create_dir(&root).expect("create config root");
    symlink(outside.join("realm"), root.join("realm")).expect("link outside realm directory");
    let before = tree(&root);

    let out = realmctl_at(&root, ["theme", "lint"]);

    assert_eq!(out.status.code(), Some(1), "{}", stderr(&out));
    assert!(stdout(&out).is_empty());
    assert!(
        !stderr(&out).contains("text.bright") && !stderr(&out).contains("text.normal"),
        "lint followed a symlinked realm directory: {}",
        stderr(&out)
    );
    assert_eq!(
        tree(&root),
        before,
        "lint must not follow a symlinked realm directory"
    );
}
...
fn lint_refuses_a_fifo_palette_promptly_without_mutating_the_config_tree() {
    let temp = tempfile::tempdir().expect("temporary config root");
    let root = temp.path().join("config");
    std::fs::create_dir_all(root.join("realm")).expect("create config root");
    let palette = root.join("realm/palette.toml");
    ...
    let mut child = Command::new(env!("CARGO_BIN_EXE_realmctl"))
        ...
        .spawn()
        .expect("realmctl starts");
    let deadline = Instant::now() + Duration::from_millis(250);
    while child.try_wait().expect("poll realmctl").is_none() {
        if Instant::now() >= deadline {
            child.kill().expect("stop blocked realmctl");
            let _ = child.wait();
            panic!("default lint blocked opening a FIFO palette");
        }
        std::thread::sleep(Duration::from_millis(10));
    }
    let output = child.wait_with_output().expect("collect realmctl output");
...
fn apply_operational_failure_exits_six_with_a_safe_diagnostic() {
    ...
    let out = realmctl_at(&root, ["theme", "apply"]);
...
fn diff_after_palette_edit_is_sorted_and_does_not_mutate_generation_tree() {
    let temp = tempfile::tempdir().expect("temporary config root");
    let root = temp.path();
    realm_theme::apply(root).expect("apply initial generation");
    edit_palette(root);
    let before = tree(root);
...
fn diff_refusal_for_missing_current_exits_six_without_mutating_generation_tree() {
    ...
    let root = temp.path();
    realm_theme::apply(root).expect("apply initial generation");
    std::fs::remove_file(root.join("realm/generated/current"))
...
fn diff_json_reports_identical_and_sorted_changes() {
    ...
    let identical = realmctl_at(root, ["theme", "diff", "--json"]);
    ...
    edit_palette(root);
    let changed = realmctl_at(root, ["theme", "diff", "--json"]);
...
fn diff_json_refusal_has_no_result_object() {
    ...
    let root = temp.path();
    realm_theme::apply(root).expect("apply initial generation");
    std::fs::remove_file(root.join("realm/generated/current")).expect("remove current pointer");
    let before = tree(root);

    let out = realmctl_at(root, ["theme", "diff", "--json"]);
