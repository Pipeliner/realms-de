...
const CHECKPOINT: &str = r#"schema = "realm-agent-sdd/checkpoint/v1"
issue = 120
reason = "handoff"
...
"#;
const EVIDENCE: &str = r#"{"id":"ev-001","ts":"2026-08-29T12:00:00Z","kind":"command","summary":"Ran focused check","command":"cargo test -p realm-core","exit_code":0,"git_head":"{head}","purpose":"reproduce result"}
"#;
...
fn run(repo: &Path, args: &[&str]) -> Output {
    Command::new(env!("CARGO_BIN_EXE_realm-sdd"))
        .current_dir(repo)
        .args(args)
        .output()
        .unwrap()
...
fn malformed_cli_reports_only_safe_cli_metadata() {
    let o = Command::new(env!("CARGO_BIN_EXE_realm-sdd"))
        .arg("gate")
        .output()
        .unwrap();
...
fn schema_rejects_every_evidence_scalar_and_collection_boundary() {
    ...
    let cases = [
        ...
        ("\"command\":\"cargo test -p realm-core\"", "\"command\":1"),
...
fn hygiene_rejects_invalid_values_in_every_string_class_without_echoing_them() {
    ...
    for rejected in command_cases {
        ...
        write_record(
            ...
            EVIDENCE
                .replace("{head}", &parent)
                .replace("cargo test -p realm-core", rejected),
        );
        assert_single_failure(gate(r.path()), "hygiene");
    }
}
...
fn valid_edge_values_for_each_string_class_are_accepted() {
    ...
    let evidence = format!(
        "{{\"id\":\"ev-01\",\"ts\":\"2024-02-29T23:59:59Z\",\"kind\":\"command\",\"summary\":\"Ran focused check\",\"command\":\"cargo test -p realm-core test_name @tag%1+2,3\",\"exit_code\":0,\"git_head\":\"{parent}\",\"purpose\":\"reproduce result\",\"path\":\"crates/realm_core.test-1\"}}\n{{\"id\":\"ev-2\",\"ts\":\"2026-08-29T12:00:00Z\",\"kind\":\"file-observation\",\"path\":\"crates/realm-core/src/ipc.rs\",\"lines\":\"999999999999999999999-1000000000000000000000\",\"git_head\":\"{parent}\",\"claim\":\"The fixture exists.\",\"summary\":\"Inspected focused lines.\"}}\n"
    );
...
fn decision_provenance_rejects_missing_later_decision_and_cross_revision_parents() {
    ...
    let invalid_sets = [
        ...
        format!("{{\"id\":\"ev-001\",\"ts\":\"2026-08-29T12:00:00Z\",\"kind\":\"decision\",\"git_head\":\"{parent}\",\"claim\":\"Choose the route.\",\"reason\":\"It is reproducible.\",\"derived_from\":[\"ev-002\"]}}\n{{\"id\":\"ev-002\",\"ts\":\"2026-08-29T12:01:00Z\",\"kind\":\"command\",\"summary\":\"Ran focused check\",\"command\":\"cargo test -p realm-core\",\"exit_code\":0,\"git_head\":\"{parent}\",\"purpose\":\"reproduce result\"}}\n"),
        ...
        format!("{{\"id\":\"ev-001\",\"ts\":\"2026-08-29T12:00:00Z\",\"kind\":\"command\",\"summary\":\"Ran focused check\",\"command\":\"cargo test -p realm-core\",\"exit_code\":0,\"git_head\":\"{old}\",\"purpose\":\"reproduce result\"}}\n{{\"id\":\"ev-002\",\"ts\":\"2026-08-29T12:01:00Z\",\"kind\":\"decision\",\"git_head\":\"{parent}\",\"claim\":\"Choose the route.\",\"reason\":\"It is reproducible.\",\"derived_from\":[\"ev-001\"]}}\n"),
    ];
...
fn malformed_cli_and_missing_record_reports_are_exact_and_read_only() {
    ...
    for args in malformed {
        let output = Command::new(env!("CARGO_BIN_EXE_realm-sdd"))
            .args(*args)
            .output()
            .unwrap();
        assert_eq!(output.status.code(), Some(3));
