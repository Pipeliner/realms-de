...
/// Minimum WCAG contrast ratio realm requires of body text on the pane
...
pub struct Accents {
    ...
    /// hermes.
    pub cyan: Rgb,
    /// Error, and ANSI colour 1/9. Not used in realm's own chrome.
    pub red: Rgb,
}
...
/// Fixed pixel metrics. realm has no border radius; `radius` exists only so a
...
pub struct Palette {
    ...
    pub name: String,
    /// `dark` or `light`; realm ships dark only for now.
    pub variant: String,
...
impl Palette {
    ...
    fn validate_ranges(&self) -> Result<()> {
        ...
        if self.metrics.radius != 0 {
            return Err(Error::PaletteRange(
                "metrics.radius must be 0: realm has no rounded corners".into(),
            ));
        }
    ...
    /// time — is why realm needs no compositor-side contrast filter.
