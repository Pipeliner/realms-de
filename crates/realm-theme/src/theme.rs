...
use realm_core::palette::Finding;
...
/// file, not to something inside the realm package.
...
/// Read the user palette for `realmctl theme lint` without creating any files.
///
/// The shipped palette is used only when the configuration root, its `realm`
...
/// An opened configuration root used to bootstrap the Realm-owned generated
...
impl ConfigRoot {
    ...
    fn load_lint_palette(&self) -> Result<Palette> {
        ...
        let realm = match openat(&self.fd, "realm", directory_flags, Mode::empty()) {
            Ok(fd) => fd,
            Err(Errno::NOENT) => return shipped_lint_palette(),
            Err(error) => {
                return Err(Error::Generation(format!(
                    "realm palette directory: {error}"
                )))
            }
        };
        let palette = match openat(
            ...
            Ok(fd) => fd,
            Err(Errno::NOENT) => return shipped_lint_palette(),
            Err(error) => return Err(Error::Generation(format!("realm palette: {error}"))),
        };
        let raw = read_open_palette(palette)?;
        Ok(Palette::from_toml(std::str::from_utf8(&raw).map_err(
            |error| Error::Generation(format!("realm palette is not UTF-8: {error}")),
        )?)?)
    }
    ...
    fn open_existing_generated_root(&self) -> Result<OwnedFd> {
        ...
        let realm = openat(&self.fd, "realm", flags, Mode::empty())
            .map_err(|error| Error::Generation(format!("realm: {error}")))?;
        let generated = openat(&realm, "generated", flags, Mode::empty())
            .map_err(|error| Error::Generation(format!("realm/generated: {error}")))?;
            ...
            || Mode::from_raw_mode(stat.st_mode).bits() != 0o700
        {
            return Err(Error::Generation(
                "realm/generated must be a current-UID mode-0700 directory".into(),
            ));
        }
        Ok(generated)
    }
...
fn read_raw_palette(config: &OwnedFd) -> Result<Vec<u8>> {
    ...
    let realm = openat(config, "realm", directory_flags, Mode::empty())
        .map_err(|error| Error::Generation(format!("realm palette directory: {error}")))?;
    let palette = openat(
        &realm,
        "palette.toml",
    ...
    .map_err(|error| Error::Generation(format!("realm palette: {error}")))?;
...
fn read_or_seed_raw_palette(config: &OwnedFd) -> Result<Vec<u8>> {
    ...
    let palette = match openat(&realm, "palette.toml", read_flags, Mode::empty()) {
        Ok(palette) => palette,
        Err(Errno::NOENT) => {
            let create_flags =
            ...
            match openat(
                ...
                Ok(palette) => {
                    ...
                    palette
                        ...
                        .map_err(|error| {
                            Error::Generation(format!("realm palette seed: {error}"))
                        })?;
                    fsync(&palette).map_err(|error| {
                        Error::Generation(format!("realm palette seed: {error}"))
                    })?;
                    fsync(&realm).map_err(|error| {
                        Error::Generation(format!("realm palette directory: {error}"))
                    })?;
                    return Ok(SHIPPED_PALETTE.as_bytes().to_vec());
                }
                Err(Errno::EXIST) => openat(&realm, "palette.toml", read_flags, Mode::empty())
                    .map_err(|error| Error::Generation(format!("realm palette: {error}")))?,
                Err(error) => {
                    return Err(Error::Generation(format!("realm palette seed: {error}")))
                }
            }
        }
        Err(error) => return Err(Error::Generation(format!("realm palette: {error}"))),
...
fn read_open_palette(palette: OwnedFd) -> Result<Vec<u8>> {
    ...
    if FileType::from_raw_mode(stat.st_mode) != FileType::RegularFile {
        return Err(Error::Generation(
            "realm palette must be a regular file".into(),
        ));
    }
    let mut raw = Vec::new();
...
fn reload_kind(reload: &Reload) -> Result<Vec<u8>> {
    match reload {
        ...
        Reload::RealmClients => Ok(b"realm-clients".to_vec()),
...
fn renderer_preimage(options: &BTreeMap<String, Vec<u8>>) -> Result<Vec<u8>> {
    ...
    let mut encoded = b"realm-theme-renderer-v1\n".to_vec();
...
mod tests {
    ...
    fn generation_apply_publishes_changed_bytes_without_writing_mutable_targets_or_a_reloader() {
        ...
        changed_palette.accent.violet = realm_core::color::Rgb::new(0xb0, 0x7a, 0xff);
    ...
    fn public_apply_fatal_candidate_preserves_current_generation() {
        ...
        let current = root.path().join("realm/generated/current");
    ...
    fn public_apply_keeps_an_existing_user_palette_and_publishes_its_derived_output() {
        let root = tempfile::tempdir().unwrap();
        let realm = root.path().join("realm");
        std::fs::create_dir(&realm).unwrap();
        let palette_path = realm.join("palette.toml");
        let mut user_palette = shipped();
        user_palette.accent.violet = realm_core::color::Rgb::new(0xb0, 0x7a, 0xff);
    ...
    fn public_apply_refuses_a_palette_symlink_without_seeding_its_destination() {
        ...
        std::os::unix::fs::symlink(&destination, realm.join("palette.toml")).unwrap();
    ...
    fn generation_diff_reports_only_outputs_affected_by_a_palette_change() {
        ...
        candidate.accent.violet = realm_core::color::Rgb::new(0xb0, 0x7a, 0xff);
    ...
    fn generation_diff_refuses_malformed_pending_and_invalid_state_without_writing() {
        for case in ["malformed", "pending", "invalid"] {
            ...
            let generated = root.path().join("realm/generated");
            match case {
                "malformed" => {
                ...
                "pending" => {
                    ...
                    changed.accent.violet = realm_core::color::Rgb::new(0xb0, 0x7a, 0xff);
                    let second =
    ...
    fn generation_diff_does_not_require_or_recreate_a_leases_directory() {
        let root = tempfile::tempdir().unwrap();
        apply_with_snapshot(root.path(), || one_output_snapshot(&shipped())).unwrap();
        std::fs::remove_dir(root.path().join("realm/generated/leases")).unwrap();
        let before = inventory(root.path());
    ...
    fn generation_apply_refuses_a_symlinked_configuration_root_ancestor_before_bootstrap() {
        ...
        assert!(
            !destination.path().join("config/realm/generated").exists(),
    ...
    fn generation_apply_rejects_unsafe_catalogue_fields_before_publication() {
        ...
        for (description, id, renderer_options, reload) in cases {
            ...
            assert!(
                !root.path().join("realm/generated/current").exists(),
    ...
    fn only_outputs_referencing_the_changed_colour_are_rewritten() {
        ...
        p.accent.violet = realm_core::color::Rgb::new(0xb0, 0x7a, 0xff);
    ...
    fn a_symlinked_staging_file_is_not_touched() {
        ...
        symlink(&victim, root.path().join("theme.conf.realm-tmp")).unwrap();
        let set = [Template {
            id: "staging-link",
            source: "x = {{ accent.violet }}\n",
            target: PathBuf::from("theme.conf"),
        ...
        assert!(
            std::fs::symlink_metadata(root.path().join("theme.conf.realm-tmp"))
                .unwrap()
                .file_type()
                .is_symlink(),
        ...
        assert_eq!(
            std::fs::read_link(root.path().join("theme.conf.realm-tmp")).unwrap(),
            victim
        );
    ...
    fn a_symlinked_palette_path_is_refused_without_touching_its_destination() {
        ...
        let victim = tempfile::tempdir().unwrap();
        let realm = root.path().join("realm");
        std::os::unix::fs::symlink(victim.path(), &realm).unwrap();
        ...
        std::fs::remove_file(&realm).unwrap();
        std::fs::create_dir(&realm).unwrap();
        let palette_link = realm.join("palette.toml");
    ...
    fn lint_palette_loader_refuses_every_static_symlinked_root_spelling() {
        ...
        let outside = tempfile::tempdir().unwrap();
        let root = parent.path().join("config");
        std::fs::create_dir_all(outside.path().join("realm")).unwrap();
    ...
    fn lint_palette_loader_uses_the_retained_root_descriptor_after_path_replacement() {
        ...
        std::fs::create_dir_all(root.join("realm")).unwrap();
        std::fs::create_dir_all(outside.join("realm")).unwrap();
        std::fs::write(
            root.join(USER_PALETTE),
            SHIPPED_PALETTE.replace("name = \"realm-void\"", "name = \"held-root\""),
        )
        .unwrap();
    ...
    fn lint_report_is_clean_for_the_shipped_palette_and_lists_hue_separations() {
        ...
        for s in &report.separations {
            assert!(
                s.degrees >= realm_core::palette::MIN_ACCENT_HUE_SEPARATION,
                "{}/{} only {:.1} deg apart",
                s.a,
                s.b,
                s.degrees
