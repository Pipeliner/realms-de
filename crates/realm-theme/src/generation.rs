...
impl LeaseRecord {
    ...
    fn parse(raw: &[u8]) -> std::result::Result<Self, String> {
        ...
        if lines.next() != Some("realm-generation-lease-v1") {
...
impl ParsedLeaseRecord {
    fn parse(raw: &[u8]) -> std::result::Result<Self, String> {
        match raw.split(|byte| *byte == b'\n').next() {
            Some(b"realm-generation-lease-v1") => Ok(Self::Process(LeaseRecord::parse(raw)?)),
            Some(b"realm-generation-lifecycle-lease-v1") => {
...
impl LifecycleLeaseRecord {
    ...
    fn parse(raw: &[u8]) -> std::result::Result<Self, String> {
        ...
        let mut lines = text[..text.len() - 1].split('\n');
        if lines.next() != Some("realm-generation-lifecycle-lease-v1") {
        ...
        match owner_kind {
            LifecycleOwnerKind::SystemdScope => {
                if process_group != 0
                    || unit != format!("realm-launch-{}.scope", launch.as_str())
...
impl PublicationOrder {
    ...
    fn parse(raw: &[u8]) -> std::result::Result<Self, String> {
        ...
        let mut lines = text[..text.len() - 1].split('\n');
        if lines.next() != Some("realm-generation-publication-order-v1") {
...
impl Manifest {
    ...
    fn parse_v1(input: &str) -> std::result::Result<Self, String> {
        ...
        if lines.next() != Some("realm-generation-manifest-v1") {
...
mod tests {
    ...
    fn lifecycle_lease_parser_requires_canonical_direct_identity() {
        ...
        let systemd_with_current_directory =
            systemd.replacen("cgroup user.slice/realm.scope\n", "cgroup .\n", 1);
    ...
    fn g6_gc_retains_stale_process_lease_when_lifecycle_evidence_is_malformed() {
        ...
        write_raw_lease_fixture(
            root.path(),
            "00000000000000000000000000000012",
            b"realm-generation-lifecycle-lease-v1\n",
        );
    ...
    fn process_death_releases_the_persistent_lock_and_recovery_discards_its_staging_tree() {
        ...
        let mut child = Command::new(std::env::current_exe().unwrap())
            ...
            .env("REALM_THEME_LOCK_HOLDER_ROOT", root.path())
            .stdout(Stdio::piped())
            .stderr(Stdio::piped())
            .spawn()
            .unwrap();
        ...
        loop {
            ...
            let locked = line.contains("REALM_THEME_LOCK_HELD");
            lines.push(line);
            if locked {
                break;
            }
        ...
        assert!(
            lines
                .iter()
                .any(|line| line.contains("REALM_THEME_LOCK_HELD")),
    ...
    fn killed_writer_at_pointer_transition_is_never_selected() {
        ...
        let mut child = Command::new(std::env::current_exe().unwrap())
            ...
            .env("REALM_THEME_LOCK_HOLDER_ROOT", root.path())
            .env("REALM_THEME_POINTER_TRANSITION", "1")
            .stdout(Stdio::piped())
            .spawn()
            .unwrap();
        let mut output = std::io::BufReader::new(child.stdout.take().unwrap());
        loop {
            ...
            if line.contains("REALM_THEME_POINTER_TRANSITION_HELD") {
                break;
            }
        }
    ...
    fn generation_lock_holder_subprocess() {
        let Some(root) = std::env::var_os("REALM_THEME_LOCK_HOLDER_ROOT") else {
            return;
        };
        let root = PathBuf::from(root);
        let store = GenerationStore::open(&root).unwrap();
        if std::env::var_os("REALM_THEME_POINTER_TRANSITION").is_some() {
            let candidate = "00000000000000000000000000000002";
            let _ = publish_at(
                &store,
                candidate,
                ...
                |checkpoint| {
                    if checkpoint == CommitCheckpoint::PointerCommitted {
                        println!("REALM_THEME_POINTER_TRANSITION_HELD");
    ...
    fn publication_order_v1_rejects_every_noncanonical_or_ambiguous_shape() {
        for malformed in [
            ...
            b"realm-generation-publication-order-v1\nnext-sequence 1",
            b"realm-generation-publication-order-v1\r\nnext-sequence 1\r\n",
            b"realm-generation-publication-order-v2\nnext-sequence 1\n",
            b"realm-generation-publication-order-v1\nnext-sequence 0\n",
            b"realm-generation-publication-order-v1\nnext-sequence 01\n",
            b"realm-generation-publication-order-v1\nnext-sequence +1\n",
            b"realm-generation-publication-order-v1\nnext-sequence -1\n",
