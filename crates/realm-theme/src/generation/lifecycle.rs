...
impl SessionRecord {
    ...
    fn parse(raw: &[u8]) -> Result<Self, String> {
        ...
        if lines.next() != Some("realm-activation-session-v1") {
...
impl LaunchRecord {
    ...
    fn parse(raw: &[u8]) -> Result<Self, String> {
        ...
        let mut lines = text[..text.len() - 1].split('\n');
        if lines.next() != Some("realm-activation-launch-v1") {
        ...
        match mode {
            ...
            OwnershipMode::Systemd => {
                if unit != format!("realm-launch-{}.scope", launch.encode())
                    || process_group != 0
                    || direct_drained
                {
...
impl ActivationRegistry {
    fn open(
        ...
        let realm = ensure_owned_directory(&state, "realm")?;
        let activation_fd = ensure_owned_directory(&realm, "activation")?;
    ...
    fn prepare(
        ...
        let (unit, process_group) = match request.mode {
            ...
            OwnershipMode::Systemd => {
                (format!("realm-launch-{}.scope", request.launch.encode()), 0)
            }
        };
...
bar-process-group 0\n",
    ...
    impl TransferFixture {
        ...
        fn new_for_owner(launch_id: &str, mode: OwnershipMode, owner_pid: u32) -> Self {
            ...
            write_mode(
                &state.path().join("realm/activation/session"),
        ...
        fn launch_path(&self) -> std::path::PathBuf {
            self._state
                .path()
                .join("realm/activation/launches")
        ...
        fn systemd_evidence(&self) -> VerifiedOwnership {
            TestOwnershipVerifier {
                proof: TestOwnershipProof::Systemd {
                    ...
                    cgroup: "app.slice/realm-launch.scope".into(),
                    cgroup_device: 11,
                    cgroup_inode: 13,
                },
            }
    ...
    fn add_systemd_running(
        ...
        let evidence = TestOwnershipVerifier {
            proof: TestOwnershipProof::Systemd {
                ...
                cgroup: format!("app.slice/realm-launch-{}.scope", launch.encode()),
                cgroup_device: 17,
                cgroup_inode: 19,
            },
        }
    ...
    fn canonical_preparing_records_accept_only_pre_adoption_ownership() {
        ...
        assert!(LaunchRecord::parse(&systemd_preparing(
            "none",
            "realm-launch-11111111111111111111111111111111.scope",
        ))
        .is_ok());
        assert!(LaunchRecord::parse(&systemd_preparing(
        ...
        .is_err());
        assert!(
            LaunchRecord::parse(&systemd_preparing("none", "realm-launch-wrong.scope")).is_err()
        );
    }
    ...
    fn terminal_launch_parser_accepts_only_producer_reachable_tuples() {
        ...
        .replace("lease-kind process", "lease-kind lifecycle")
        .replace("cgroup none", "cgroup app.slice/realm")
    ...
    fn open_initializes_descriptor_safe_activation_root() {
        ...
        for directory in [
            root.path().join("realm"),
            root.path().join("realm/activation"),
            root.path().join("realm/activation/launches"),
        ] {
        ...
        let lock =
            fs::symlink_metadata(root.path().join("realm/activation/lifecycle.lock")).unwrap();
        assert!(lock.is_file());
    ...
    fn open_rejects_symlinked_or_wrong_mode_lock_without_replacement() {
        for symlinked in [true, false] {
            let root = tempfile::tempdir().unwrap();
            let activation = root.path().join("realm/activation");
            fs::create_dir_all(&activation).unwrap();
            fs::set_permissions(root.path().join("realm"), fs::Permissions::from_mode(0o700))
                .unwrap();
    ...
    fn session_claim_is_exact_and_active_reopen_revalidates_live_identity() {
        ...
        write_mode(
            &root.path().join("realm/activation/session"),
            &exact_session_record("active", 7),
            0o600,
        );
        let registry = open_test_registry(root.path()).unwrap();
        ...
        let stale = String::from_utf8(exact_session_record("active", 7))
            ...
            .into_bytes();
        write_mode(&root.path().join("realm/activation/session"), &stale, 0o600);
    ...
    fn cross_namespace_temporaries_are_retained_and_fail_closed() {
        for activation_name in [true, false] {
            ...
            let path = if activation_name {
                root.path().join(
                    "realm/activation/.launch-create-bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb-cccccccccccccccccccccccccccccccc",
                )
            } else {
                root.path().join(
                    "realm/activation/launches/.session-claim-aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa",
                )
            };
    ...
    fn inventory_retains_unsafe_temporary_and_fails_closed() {
        ...
        let temporary = root.path().join(
            "realm/activation/launches/.launch-create-bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb-cccccccccccccccccccccccccccccccc",
        );
    ...
    fn temporary_inode_replacement_before_delete_is_retained() {
        ...
        let temporary = root
            .path()
            .join("realm/activation/.session-claim-aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa");
        let displaced = root.path().join("realm/activation/displaced-temporary");
    ...
    fn over_bound_inventory_retains_every_safe_temporary() {
        let root = tempfile::tempdir().unwrap();
        drop(open_test_registry(root.path()).unwrap());
        let launches = root.path().join("realm/activation/launches");
        for index in 0..=MAX_INVENTORY_ENTRIES {
    ...
    fn prepare_publishes_exact_gate_closed_selection_identity() {
        ...
        assert!(!state
            .path()
            .join("realm/activation/launches/66666666666666666666666666666666")
            .exists());
    }
    ...
    fn successful_transfers_persist_exact_direct_and_systemd_evidence() {
        ...
        assert_eq!(adopted.record.cgroup, "app.slice/realm-launch.scope");
    ...
    fn pre_exchange_crash_subprocess() {
        let Ok(generated) = std::env::var("REALM_TEST_CRASH_GENERATED") else {
            return;
        };
        let state = std::env::var("REALM_TEST_CRASH_STATE").unwrap();
    ...
    fn real_partial_stage_write_crash_never_publishes_named_stage() {
        ...
        let status = std::process::Command::new(std::env::current_exe().unwrap())
            ...
            .env("REALM_TEST_CRASH_GENERATED", generated.path())
            .env("REALM_TEST_CRASH_STATE", state.path())
            .status()
            .unwrap();
        ...
        let crashed = LaunchRecord::parse(
            &fs::read(
                state
                    .path()
                    .join("realm/activation/launches/77777777777777777777777777777777"),
            )
            .unwrap(),
        )
        .unwrap();
    ...
    fn real_pre_exchange_crash_requires_registry_reconciliation_before_a_new_transfer() {
        ...
        let crashed_path = state
            .path()
            .join("realm/activation/launches/88888888888888888888888888888888");
    ...
    fn valid_transfer_stage_is_not_normalized_when_unrelated_evidence_is_fatal() {
        for fatal_launch in [
            ...
            let safe_temporary = fixture._state.path().join(
                "realm/activation/launches/.launch-create-22222222222222222222222222222222-33333333333333333333333333333333",
            );
    ...
    fn restart_reconciliation_uses_descriptor_capability_not_state_config_path() {
        ...
        drop(fixture.store);
        let decoy_generated = state_path.join("realm/generated");
