...
impl Reloader for SystemReloader {
    fn reload(&mut self, reload: &Reload) -> Result<()> {
        match reload {
            ...
            // The control socket arrives with realm-session in M2. Nothing in
            // the M1 template set uses this, and saying so is better than a
            // silent no-op that looks like a working reload.
            Reload::RealmClients => Err(Error::Reload(
                "realm's control socket does not exist until M2".into(),
            )),
        }
    }
}
...
fn signal_all(process: &str, signal: i32) -> Result<()> {
    ...
    // unsafe, so the signals realm is willing to send are named rather than
    ...
    let signal = match signal {
        ...
        other => return Err(Error::Reload(format!("realm will not send signal {other}"))),
    };
