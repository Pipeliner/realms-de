...
**Goal:** Make theme application write only Realm-owned files while safely activating GTK themes on first run.

**Architecture:** `realm-theme` renders every shipped target below `realm/generated/`. A small activation model records the user-side GTK file and its exact import line; it creates that file only when absent and otherwise reports the required remedy without modifying it. The later `realmctl doctor` implementation (#23) consumes this model rather than reconstructing paths and wording.

**Tech Stack:** Rust 2021, existing `realm-theme` descriptor-relative writer, Rust 1.85.

**Spec:** `docs/specs/0002-theme-pipeline.md`, “Generated-file ownership and activation”.

## Global Constraints

- Realm owns only `$XDG_CONFIG_HOME/realm/generated/**` and may replace only files there.
...
**Files:**
- Modify: `crates/realm-theme/src/template.rs`
- Modify: `crates/realm-theme/src/theme.rs`

**Interfaces:**
- Produces a public, data-only activation diagnostic suitable for #23.

- [ ] Add failing tests proving every shipped template target starts with `realm/generated/`.
...
- [ ] Redirect all shipped targets to `realm/generated/<tool>/...` without changing template source or reload identity.
...
- [ ] Run Task 1 tests red-to-green and the full `realm-theme` crate suite.
