...
**Architecture:** Keep generation ownership in `realm-theme`. Add a typed lifecycle-lease record and dispatch generic GC by its discriminator before liveness; every present lifecycle lease pins its generation and freezes all lease and generation deletion for that pass. The later `realm-session` M2 component owns the activation registry, lock ordering, adoption, atomic transfer, and systemd/direct proof without creating a dependency from `realm-theme` back to session state.
...
**Files:**
- Modify: `crates/realm-theme/src/generation.rs:112-120,1832-1905`
- Test: `crates/realm-theme/src/generation.rs:2953-3025,3236-3310`
...
**Files:**
- Modify: `crates/realm-theme/src/generation.rs:920-1080`
- Test: `crates/realm-theme/src/generation.rs:3236-3460`
...
Run: `cargo test -p realm-theme g6_gc_retains_lifecycle_lease -- --nocapture`
