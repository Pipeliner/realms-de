# Realmctl Theme CLI Implementation Plan
...
**Goal:** Ship the `realmctl theme apply`, `lint`, and `diff` executable surface required by SPEC 0002, SPEC 0006, and SPEC 0011.

**Architecture:** Add a small `crates/realm-ctl` binary crate which parses only the theme command group with `clap` and calls `realm_theme::{apply,diff,lint,load_palette}` in-process. It owns configuration-root and optional palette selection plus human diagnostics/exit mapping; it does not open an IPC socket, reload processes, alter the sealed-generation protocol, or implement the wider M2 CLI surface.

**Tech Stack:** Rust 1.85, `clap` derive from workspace dependencies, `realm-theme`, `tempfile` integration fixtures, `std::process::Command`.

**Spec:** `docs/specs/0002-theme-pipeline.md` A6/A9/A10; `docs/specs/0006-realm-ctl.md` §§2/4 and B1–B3/B15–B17; `docs/specs/0011-theme-activation-generations.md` G10–G12.

## Global Constraints

- The installed binary name is exactly `realmctl`; do not claim or install a `realm` alias.
...
- Configuration root is `--config-root PATH` when supplied; otherwise `$XDG_CONFIG_HOME`, otherwise `$HOME/.config`. A missing `HOME` is a usage failure. `--palette PATH` is an explicit read-only lint input; apply/diff use the secure Realm-owned `realm/palette.toml` path until a separate accepted input-snapshot API exists.
...
**Files:**
- Modify: `Cargo.toml`
- Create: `crates/realm-ctl/Cargo.toml`
- Create: `crates/realm-ctl/src/main.rs`
- Create: `crates/realm-ctl/tests/theme_cli.rs`

**Interfaces:**
- Consumes: `clap::{Parser,Subcommand}` and `std::process::ExitCode`.
- Produces: executable `realmctl` with `theme { apply | lint | diff }`; `run(args, env) -> ExitCode` is unit-testable without process-global mutation.
...
fn rejects_missing_or_unknown_theme_verb() {
    assert_eq!(run_from(["realmctl", "theme"]), ExitCode::from(2));
    assert_eq!(run_from(["realmctl", "theme", "reload"]), ExitCode::from(2));
}
```

- [ ] **Step 2: Run the test to verify it fails**

Run: `cargo test -p realm-ctl rejects_missing_or_unknown_theme_verb`

Expected: FAIL because package `realm-ctl` does not exist.
...
Expected: PASS; `realmctl theme --help` documents only `apply`, `lint`, and `diff`.

- [ ] **Step 5: Commit the isolated parser slice**

```bash
git add Cargo.toml crates/realm-ctl
git commit -m "feat: add realmctl theme command parser"
```
...
- Modify: `crates/realm-ctl/tests/theme_cli.rs`

**Interfaces:**
- Consumes: `realm_theme::load_palette`, `realm_theme::lint`, `realm_core::Palette::load`.
...
fn lint_shipped_palette_is_session_independent_and_prints_hue_separations() {
    let out = realmctl(["theme", "lint", "--config-root", temp.path()]);
...
fn lint_bad_explicit_palette_exits_one_and_names_finding() {
    ...
    let out = realmctl(["theme", "lint", "--palette", bad.to_str().unwrap()]);
...
Run: `cargo test -p realm-ctl lint_`
...
Run: `cargo test -p realm-ctl lint_ -- --nocapture`
...
```bash
git add crates/realm-ctl
git commit -m "feat: add realmctl theme lint"
```
...
**Interfaces:**
- Consumes: `realm_theme::apply`, `realm_theme::diff`, `realm_theme::ThemeOutputChange`.
...
fn diff_after_palette_edit_is_sorted_and_does_not_mutate_generation_tree() {
    let root = temp.path();
    assert!(realmctl_at(root, ["theme", "apply"]).status.success());
    edit_palette(root, "accent.violet", "#b07aff");
    let before = tree(root);
    let out = realmctl_at(root, ["theme", "diff"]);
    assert_eq!(out.status.code(), Some(1));
...
Run: `cargo test -p realm-ctl diff_after_palette_edit_is_sorted_and_does_not_mutate_generation_tree`
...
Run: `cargo test -p realm-ctl diff_ && cargo test -p realm-theme generation_diff_`
...
```bash
git add crates/realm-ctl
git commit -m "feat: add realmctl theme diff"
```
...
**Interfaces:**
- Consumes: `realm_theme::apply` and `GenerationPublicationOutcome::{Committed,CommittedWithCleanupPending,OutcomeAmbiguous}`.
...
fn public_apply_fatal_candidate_preserves_current_generation() {
    ...
    assert!(realm_theme::apply(root.path()).is_err());
...
Run: `cargo test -p realm-ctl apply_ && cargo test -p realm-theme public_apply_fatal_candidate_preserves_current_generation`
...
```bash
git add crates/realm-ctl crates/realm-theme/src/theme.rs
git commit -m "feat: add realmctl theme apply"
```
...
- Modify: `packaging/debian/realm.install`
- Modify: `packaging/fedora/realm.spec`
- Modify: `docs/specs/0002-theme-pipeline.md`
- Modify: `docs/specs/0006-realm-ctl.md`

**Interfaces:**
- Consumes: cargo binary `target/release/realmctl`.
- Produces: all packaging lanes install `realmctl`; accepted-spec acceptance tables name real test evidence.

- [ ] **Step 1: Write failing packaging/projection and CLI install checks**

```bash
rg -n 'realmctl' packaging/nix/package.nix packaging/debian/realm.install packaging/fedora/realm.spec
cargo build -p realm-ctl --release --locked
test -x target/release/realmctl
```
...
Use each packaging format’s existing binary-install convention. Do not add a `realm` binary, package publication channel, network job, daemon, or alias. Replace only B1–B3/B15–B17 test-table blanks with the actual `realm-ctl` integration test names.

- [ ] **Step 3: Run complete local verification**

Run: `cargo fmt --all -- --check && cargo clippy --workspace --all-targets --all-features -- -D warnings && cargo test --workspace --all-features --locked && cargo build -p realm-ctl --release --locked && bash packaging/fedora/check-projections.sh`
...
```bash
git add Cargo.toml crates/realm-ctl crates/realm-theme/src/theme.rs packaging docs/specs
git commit -m "feat: ship realmctl theme commands"
```
