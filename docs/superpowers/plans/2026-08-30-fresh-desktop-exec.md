...
- The owner remains supervisor; exactly one profile child changes cwd/stdin/env, close-sweeps internal descriptors, and calls safe wrapped `fexecve`.  A complete valid fixed error frame proves unavailable/returned pre-replacement execution failure and makes the owner reap the exact child then write the existing terminal-failure witness.  A short/malformed/partial frame or CLOEXEC EOF/no report is neither execution-failure nor successful-exit proof: after exact-child reap it records `terminal/lost` uncertainty unless a separately accepted post-replacement witness exists; independent empty-ownership/drain proof may still release the lease.
