# Packaged `realm-sdd` Git Runtime Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Make the Nix-installed `realm-sdd` resolve Git from its package
runtime path and prove that its session wrapper does not receive Git.

**Architecture:** The unified `realm` output obtains a Git store reference only
through a `realm-sdd` wrapper. A focused Nix check creates a clean disposable
Git record-carrier, invokes that installed wrapper with an unusable inherited
`PATH`, and inspects the separate `realm-session` wrapper for Git's absence.
...
existing Rust `realm-sdd` fixture contract.

**Spec:** [SPEC 0010](../../specs/0010-packaged-realm-sdd-git-runtime.md),
[ADR 0016](../../adr/0016-packaged-realm-sdd-carries-git.md)

## Global Constraints

- Wrap `realm-sdd` with `pkgs.git` only; do not add Git to
  `support.wrapperRuntime` or `realm-session`'s wrapper path.
...
**Interfaces:**
- Consumes: `realm`, `pkgs.git`, `pkgs.coreutils`, and the source tree passed to
  the existing checks function.
- Produces: `checks.<system>.realm-sdd-git-runtime`, a derivation that exits
...
- [x] **Step 1: Add the regression before the wrapper change**
  ...
  ${pkgs.coreutils}/bin/env -i PATH=/nonexistent \
    ${realm}/bin/realm-sdd gate --issue 120 --from probe --to spike \
    > "$TMPDIR/actual.json"
...
- [x] **Step 2: Establish the red condition without pushing a red branch**
  ...
  `realm-sdd` invocation because `PATH=/nonexistent` contains no Git. If this
...
- [x] **Step 3: Verify CI executes the check on every runner path**
  ...
  ```bash
  python3 -c 'from pathlib import Path; text = Path(".github/workflows/distro.yml").read_text(); start = text.index("nix build --print-build-logs"); block = text[start:text.index("fi", start)]; assert ".#checks.$system.realm-sdd-git-runtime" in block'
...
### Task 2: Wrap only `realm-sdd` and correct stale package comments
...
- Produces: `$out/bin/realm-sdd` whose wrapper prefixes the Git store bin path;
  `$out/bin/realm-session` keeps its existing `support.wrapperRuntime` path.
...
- [x] **Step 2: Correct stale pre-alpha comments**
  ...
  has `realm-core` and installs no Realm binaries. State precisely that desktop
  binaries remain pending while the local `realm-sdd` validator is installed.
...
- [x] **Step 3: Run green verification**
  ...
  ```bash
  git diff --check
  cargo test --offline -p realm-agent-sdd --test gate -- --test-threads=1
  nix-instantiate --parse flake.nix >/dev/null
  nix flake check --print-build-logs
  ```

  The final command is authoritative and may run remotely when the local Nix
  version cannot evaluate the lock. Require `realm-sdd-git-runtime` and the
...
- [x] **Step 4: Commit**
  ...
  git add packaging/nix/checks.nix packaging/nix/package.nix flake.nix \
    docs/superpowers/plans/2026-08-29-packaged-realm-sdd-git-runtime.md
  git commit -m "fix: wrap packaged realm-sdd with Git (#146)"
  ```

### Task 3: Review, land, and record proof

**Files:**
- Modify: `docs/specs/0010-packaged-realm-sdd-git-runtime.md`
