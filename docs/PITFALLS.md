...
| `realm-session` stalls | Keys stop responding; River v0.4.8 eventually drops new input after its bounded queue fills | Nothing on the input path may block — no theme apply, no write to a wedged subscriber. §4 budgets are correctness bounds here | *planned:* M2 latency test asserting no handler exceeds its budget |
...
| A policy response fails after River may have applied part of it | Continuing could publish desired state that never became compositor truth | The MVP fails the session closed, emits no ordinary completion/effect, and relies on supervised restart plus River replay; in-incarnation repair is post-MVP | *planned:* `session::tests::post_admission_io_and_unsupported_fail_closed_without_completion_or_effects` |
