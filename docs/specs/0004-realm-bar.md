...
The bar is the only place realm tells the user what state it is in. Which orbit
...
**Out:** producing `RealmState` at all — that is `realm-session`, which owns the
...
(ADR 0013, `realm-session`). Generating the palette (`realm-theme`, SPEC 0002).
...
`RealmState`; the keys that toggle the strip and the grimoire are ordinary
keymap bindings (`Action::ToggleWhichKey`, `Action::Grimoire`) resolved by
`realm-session`, which flips a field in the state and broadcasts it. A bar that
grabbed the keyboard would be a second input path, and realm has one.
...
`river-layer-shell-v1`, and realm's window manager is `realm-session`
...
`realm-session` as a free rectangle in global coordinates and is converted into
a `Workarea` by the backend ([INTERFACES.md §1](../INTERFACES.md)).
`realm-session` uses that rectangle as given. It must not *also* subtract
...
so there is no node order for `realm-session` to pick. On `Top` the bar would
draw over a fullscreen video; on `Bottom` the window covers it, which is what
`mod+f` is asking for.

`Bottom` costs nothing the rest of the time, because realm *is* the window
...
`realm_core::Palette`; a literal in the bar is the same bug as a colour outside
...
**Logo.** `✦ realm` in `accent.violet` at `typography.size_body`, 14 px padding
...
**Six orbit runes.** Source of truth: `RealmState::orbits`, always six
...
Source of truth: `RealmState::layout`. `label()` yields `triptych`, `mono` or
...
`RealmState::mode`. `badge()` yields the bare word — `NAV`, `RESIZE`, `MOVE` —
and the bar prepends the resolved `⌨`.

**Centre: focused title and chord echo.** A flexible region, contents centred,
14 px between the two. The title is `RealmState::focused_title` verbatim in
`text.soft`. The chord echo is `RealmState::chord_echo` in `text.faint` at
...
**Right-hand modules.** Source of truth: `RealmState::modules`, in the order the
...
Visibility is `RealmState::whichkey`. False destroys the surface (§1).
...
- Source of truth: `RealmState::grimoire` (a new field, see
  [Additions required in realm-core](#additions-required-in-realm-core)) for
...
**Pure function.** For the same `(RealmState, Palette, Probe)` and the same
...
`RealmState::renders_same_as` compares everything visible and ignores
...
([SPEC 0003](0003-realm-session.md) §8), so in a correct session this check
...
sample. The ladder is the `realm-perf-budget` skill's: an event if one exists, a
...
|---|---|---|---|
| clock | wall clock | `timerfd` scheduled to the next minute boundary in `realm-session` | minute-aligned display update (ADR 0009) |
...
### Additions required in `realm-core`

None of these are workarounds; the types do not yet carry what the bar needs.
Each is a revision of [SPEC 0001](0001-realm-core-contracts.md), made in the
same commit as the change, per `docs/specs/README.md`.

| Addition | Why |
|---|---|
| `RealmState::grimoire: bool`, compared in `renders_same_as` | `Action::Grimoire` exists in `keys.rs`, but no field tells the bar the sheet is open. Without it the bar cannot draw the grimoire at all |
...
| `ipc::Request::GetKeymap` → `ipc::Response::Keymap(Box<Keymap>)` | The which-key strip and the grimoire are both rendered from `Keymap`, which the session owns and the bar has no way to ask for. It does not belong in `RealmState`: it changes only when the configuration does, and putting it in a per-change broadcast would add kilobytes to every frame's worth of traffic. The bar fetches it once after `Hello` |
...
It contributes to, but does not own: **layer-shell not served** (realm-session
