...
- **Status:** Accepted (2026-08-26; generation contract reconciled by #159;
  ...
  healthy-session `doctor` VM acceptance is retained, while the B12
  missing-activation VM case and numeric tool floors remain open; the other
...
| B11 | Given a healthy fresh-login session, when `doctor` runs, then every resolved check reports `ok` or `warn`, idle lock, the unrequested file-chooser round trip and unresolved tool floors report their accepted explicit `skip`, the header names the tool, protocol, distribution, kernel, backend and negotiated compositor interfaces without inventing a compositor package version, and it exits 0. The session entry starts portal activation non-blockingly after importing its environment, rather than making the fixture warm it manually; the installed-package fixture invokes `realmctl` without an interactive-shell path and proves its permitted `gsettings` and reused-tool probes remain reachable. The VM writes the exact JSON report and canonical CI retains it in the live-evidence artifact. | `session-boots` NixOS VM on exact head `a46940242304fabd7b3336eafe3777603b8eb03a` (canonical run `34752320429`, job `103710854254`; retained artifact `10315654053`, `realmctl-doctor.json` SHA-256 `11ea280d0ee0e954cca084e8b5fc3701f43ca170e6899f0313a2d179b139079c`); `realmctl-command-runtime` Nix package check |
| B12 | Given a live Realm session and two independent fresh D-Bus session buses whose daemons inherited none of the graphical-session variables and do not delegate activation to systemd, when the control bus receives the exact non-systemd `dbus-update-activation-environment` import from SPEC 0005 while the omitted-import bus does not, and the same installed `doctor` activates the same real portal frontend, selected backend and configuration on each bus, then the control `env/wayland-display/dbus` check passes while the omitted-import check fails within its 2 s deadline, prints the 25-second-hang symptom, reports the observed portal failure without claiming to have read a missing variable, names the activation environment, portal service and selected backend as possible causes, prints the `dbus-update-activation-environment` remedy, and exits 1. Because the report does not expose per-probe elapsed time, the VM guards the complete invocation at 5 s (the accepted 3 s doctor budget plus 2 s of fresh-bus and transient-unit overhead), while production's shared absolute 2 s bus deadline remains covered by B14b. The retained whole-invocation elapsed value is guard evidence only, not a measurement of the individual portal probe. Each probe runs in a transient control group with a 1 s stop grace that is killed on its own deadline so an activation failure cannot leave children behind. Before applying semantic or status assertions, the VM writes every available report and command stderr plus metadata carrying distinct bus identities, advertised features, statuses and observed elapsed times; canonical CI retains them in the live-evidence artifact. This paired real-backend probe composes with the session-entry ordering test that requires the production import before portal warm-up. It does not claim that omission must remain observable on the supported systemd topology: the separate systemd-manager import followed by explicit portal service start can correctly supply the same variables and mask that omission. If the omitted-import portal property still answers, the VM probe is insufficient and this row remains open; the test must not manufacture a second failure in the portal unit or weaken the diagnostic expectation. | `realmctl::doctor::tests::portal_failure_reports_an_observation_and_only_possible_causes` (diagnostic mapping only; paired direct-activation VM case pending) |
