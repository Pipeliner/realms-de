...
`realm-wm.service` environment; it does not rely on the system profile or an
interactive-shell PATH that the user service overrides.
...
lease PID, sends Escape through the compositor, and observes that exact owned
process disappear. Merely finding Foot/Fuzzel in the unit PATH is not runtime
evidence for either fixed-consumer path.
