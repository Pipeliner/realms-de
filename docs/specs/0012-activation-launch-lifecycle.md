...
- **Status:** Accepted (2026-08-30; fixed-consumer terminal-descendant
...
fixed-consumer path. The installed fixture reads the running daemon's `/proc`
environment rather than inferring the effective value from one unit source.
Neither path relies on the system profile or an interactive-shell PATH that the
user service overrides.

The authoritative installed-session fixture drives River's real default
`mod4+Return` and `mod4+d` bindings rather than substituting a control `Spawn`.
For terminal it observes the managed Foot window, the exact selected-N config
argument and terminal override, and an ordinary process lease naming that Foot
PID. For launcher it observes the exact selected-N Fuzzel config argument and
lease PID, then waits for a deterministic fixture desktop-entry label to be
rendered before sending Escape through the compositor and observing that exact
owned process disappear. Process existence alone is not surface/readiness
evidence. Merely finding Foot/Fuzzel in the unit PATH is not runtime evidence
for either fixed-consumer path.
...
foreground TUI descendants may dereference the same N, but they receive no
lifecycle or survival claim; their selected files remain available solely
through SPEC 0011's production no-reclamation M1 rule. Re-enabling production
GC requires this path to adopt the ownership and lifecycle lease below first.
Fuzzel children receive no generation selector. This is not authority to route
a desktop entry or arbitrary
...
| `running`, lifetime owner receives a complete pre-image-replacement error report that `fexecve` returned and reaps its exact profile child | lifetime owner only | `terminal/failed`, lifecycle lease | no profile code ran. The owner reads the child-only close-on-exec error channel and revalidates/reaps that exact child before record mutation. For systemd, fsync terminal record then exit; reconciler later proves recursive scope emptiness before unlink/fsync. For direct, prove zero attributed descendants, fsync `terminal/failed direct-drained yes` before owner exit; reconciler later proves exact owner stale and group empty before unlink/fsync. |
| `running`, descriptor-executed profile child has a short/malformed/partial error frame or EOF without a complete pre-image-replacement report, and no separately accepted post-replacement witness | lifetime owner only | `terminal/lost`, `direct-drained yes` after exact drain for direct, lifecycle lease | revalidate/reap the exact child, prove the existing required empty ownership/drain condition, and fsync the uncertain terminal record. It makes no claim whether profile code ran; the ordinary independent empty-ownership/drain proof, not that claim, controls later lease release. |
| `running`, systemd scope reaches complete recursive emptiness | registry reconciler | `terminal/exited`, lifecycle lease | prove recursive emptiness, fsync terminal record, then unlink/fsync lease. For a descriptor-executed profile, this row requires either a complete pre-replacement report (which instead uses `terminal/failed`) or a separately accepted post-replacement witness; otherwise the preceding `terminal/lost` row applies. |
| direct owner has reaped all attributed descendants and is still exact/live | lifetime owner only | `terminal/exited`, `direct-drained yes`, lifecycle lease | fsync terminal record before the owner exits; a later reconciler proves exact owner stale and group empty before unlinking/fsyncing lease. For a descriptor-executed profile, this row requires either a complete pre-replacement report (which instead uses `terminal/failed`) or a separately accepted post-replacement witness; otherwise the preceding `terminal/lost` row applies. |
...
| `terminal/lost`, `lease-kind lifecycle` | matching lifecycle lease | Do not infer image replacement or profile-code execution from the child's EOF, exit status, or terminal state. For systemd, independently re-prove complete ownership empty; for direct, validate `direct-drained yes`, prove exact owner staleness, then prove the recorded group empty. Only that proof permits unlink/fsync and collection. |
| `terminal/lost`, `lease-kind lifecycle` | lease absent | Do not infer image replacement or profile-code execution from absence. For systemd, independently re-prove recorded ownership empty; for direct, validate `direct-drained yes`, exact owner staleness, and recorded-group emptiness. Only that proof permits collection; otherwise retain the record as uncertain and report fatal/uncertain. |
