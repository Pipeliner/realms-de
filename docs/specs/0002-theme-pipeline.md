...
| helm's own clients | unsuppressible `Event::ThemeReloaded` over the control socket |
...
| A7 | Given a successful apply that changes an output, when the reload fan-out runs, then each reload mechanism fires exactly once regardless of how many templates share it, and `Event::ThemeReloaded` is not suppressed by state-frame coalescing | `theme::tests::each_reload_mechanism_fires_exactly_once`, `theme::tests::theme_reload_event_is_not_coalesced_with_state` |
