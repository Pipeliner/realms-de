...
| A3 | Given a tracked automation fixture containing any helper byte change; a forbidden direct body publication form; a bare, conditional, or command-prefixed helper GitHub CLI invocation; or single-line, multiline, or POSIX backslash-newline-joined helper command substitution, when the repository guard runs, then it fails; given the SHA-256-pinned helper and the approved read-only `scripts/ci-monitor`, it passes. | `docs/test-github-body-safety.sh` — `tracked-command-allowlist` |
