...
| A2 | Given invalid identifiers, arity, non-regular/unreadable body inputs, leading-hyphen body paths, or title files with empty, unterminated, multiline, CR, NUL, or operand-looking path input, when the helper is run, then it fails before invoking the fake GitHub CLI. | `docs/test-gh-body-file.sh` — `reject-before-gh` |
