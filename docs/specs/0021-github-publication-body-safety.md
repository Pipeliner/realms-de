...
2. The helper supports `issue-create`, `issue-comment`, `issue-edit`, and
   `pr-create` with fixed argument positions. Issue numbers are positive decimal
   identifiers; branch references are nonempty and do not begin with `-`;
   ...
   body files fail before any GitHub CLI invocation. The safety checker rejects
...
| A2 | Given invalid identifiers, arity, non-regular/unreadable body inputs, or title files with empty, unterminated, multiline, CR, NUL, or operand-looking path input, when the helper is run, then it fails before invoking the fake GitHub CLI. | `docs/test-gh-body-file.sh` — `reject-before-gh` |
| A3 | Given a tracked automation fixture containing any forbidden direct body publication form or single-line or multiline helper command substitution, when the repository guard runs, then it fails; given the approved helper and read-only queries, it passes. | `docs/test-github-body-safety.sh` — `tracked-command-allowlist` |
