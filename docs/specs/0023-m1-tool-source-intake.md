...
Realm supports Yazi and Starship on all three M1 targets without a live upstream
download during package construction and without Realm-operated public package,
...
3. The retained inputs preserve the upstream MIT (Yazi) and ISC (Starship)
   notices. Realm does not claim that a checksum alone authenticates an upstream
...
5. This contract does not publish a package repository, key, binary, container,
   ...
   repository as a supported Realm source. The native packages declare or build
   from the retained inputs; an active immutable Realm generation remains
...
| A4 | Given an installed immutable Realm generation, when package input update or rollback is selected, then its selected generation bytes are unchanged; only later launches may use the changed package/runtime. | Not yet implemented; requires generation-lifecycle evidence and a rollback fixture. |
