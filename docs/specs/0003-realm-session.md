...
locked state in report order and removes every action-producing input event
...
| A9c | Given an enabled repeatable Realm binding, when River reports either (a) its press and then `session_locked` before the same unfinished policy turn reaches `manage_start`, or (b) a press in a completed turn followed by release/stop and `session_locked` in the next unfinished turn, then the lock turn removes every queued action-producing input event so it cannot launch, retains or synthesizes exactly one repeat-stop for every id that might have armed Session repeat, and its response disables every configured binding, clears modifier watching and cancels a pending eaten-key edge regardless of Session's desired state. Binding/modifier/chord events received after the lock event are suppressed while authoritative non-input facts retain report order. When River later reports `session_unlocked`, the following response restores Session's exact desired binding and modifier state | #40 `backend::tests::session_lock_disables_and_suppresses_realm_bindings_until_unlock` |
