...
- **Status:** Accepted (2026-09-10; control-transport and nonblocking backend
  ...
  clarification 2026-09-12; MVP module-producer correction and doctor health
  boundary 2026-09-13)
- **Milestone:** M2
...
The default Return binding is `Action::Terminal`, not a raw Spawn containing a
helper name. Session reduces it to `SessionEffect::Terminal`; only the runtime
adapter maps that closed effect to the fixed consumer executor specified by
SPEC 0012. Consequently `GetKeymap` reports the typed action and neither a
client-supplied Spawn argv nor an argv string that happens to equal an installed
helper can select the fixed-consumer path.
...
River's `session_locked` / `session_unlocked` events are a backend-local input
gate, not a second Session mode. On `session_locked`, the backend records the
locked state in report order and immediately exposes its read-only binding-
input-suspended state to the outer runtime, without waiting for the following
`manage_start`. Before servicing any simultaneously ready repeat expiry, the
runtime observes that state, disarms the repeat timer and consumes any retained
readiness without calling `Session::fire_key_repeat()`. The gate remains closed
until the corresponding `session_unlocked` reaches its policy boundary. This
immediate gate is private transport state: the ordered policy turn remains the
only way Session learns the lock fact. The backend removes every action-producing input event
already queued in that same unfinished policy turn. For every binding which
might have armed Session repeat—whether held from an earlier completed turn or
pressed, released or stopped in the unfinished turn—it retains or synthesizes
exactly one `BindingRepeatStopped`, then clears its held set. Thus a press
immediately before the lock event cannot launch after the compositor has
locked, while a release immediately before it cannot strand an earlier repeat.
In the response to the
following `manage_start`, every configured binding is effectively disabled,
modifier watching is cleared and any pending eaten-key edge is cancelled even
when Session's desired response still names enabled bindings. While locked,
binding press/release/repeat-stop, eaten-key and modifier events are consumed
without reaching Session; authoritative window/output/focus facts retain their
ordinary order. On `session_unlocked`, the following response again applies
Session's complete desired binding state. This keeps lock state compositor-
owned, disarms Realm's repeat path at the boundary and does not invent a
lock-screen binding handled by Realm.
...
**The specified architecture: one event-loop thread, one worker thread, one
shared module-sampler thread, and one bounded child reaper.**
...
`binding_input_suspended` is a read-only, infallible snapshot owned by the
backend. River changes it immediately when dispatch observes `session_locked`,
before the later public policy turn exists, and clears it only at the matching
unlock policy boundary. The runtime consults it only to prevent a previously
armed Realm repeat from firing across that private protocol gap; it does not
derive Session mode, publication or compositor response state from it.
...
| A9 | Given `Keymap::default()`, when bindings are configured and the initial replay policy turn is answered, then one `river_xkb_binding_v1` mechanism exists per stable id with the xkbcommon keysym and canonical modifiers, Session alone maps ids to policy, and that projection-free response enables none with next-key Preserve. The first later selected-output workarea response enables exactly the `Mode::Nav` ids. When its Return id is pressed, the successful final boundary emits exactly `SessionEffect::Terminal`, never `SessionEffect::Spawn`; `GetKeymap` exposes the same typed action | `session::tests::binding_configuration_contains_mechanism_not_policy`, `session::tests::initial_workarea_enables_nav_bindings_after_disabled_replay`, `session::tests::default_return_binding_emits_only_the_typed_terminal_effect`; `runtime::tests::request_dispatch_uses_live_state_and_exact_session_keymap`; #40 `backend::tests::configured_bindings_create_one_stable_river_object_each` |
...
| A9c | Given an enabled repeatable Realm binding, when River reports either (a) its press and then `session_locked` before the same unfinished policy turn reaches `manage_start`, or (b) a press in a completed turn followed by release/stop and `session_locked` in the next unfinished turn, then observing the lock immediately closes the backend's repeat gate before any ready timer can call Session, and the lock turn removes every queued action-producing input event so it cannot launch, retains or synthesizes exactly one repeat-stop for every id that might have armed Session repeat, and its response disables every configured binding, clears modifier watching and cancels a pending eaten-key edge regardless of Session's desired state. Binding/modifier/chord events received after the lock event are suppressed while authoritative non-input facts retain report order. The gate remains closed until `session_unlocked` reaches its policy boundary; that following response restores Session's latest exact desired binding and modifier state rather than a pre-lock cache | #40 `backend::tests::session_lock_disables_and_suppresses_realm_bindings_until_unlock`; #38 `runtime::tests::backend_observed_lock_disarms_repeat_before_delayed_policy_boundary` |
