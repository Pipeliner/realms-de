...
locked state in report order, removes every input event already queued in that
same unfinished policy turn, emits one `BindingRepeatStopped` for each binding
it had reported pressed and clears that held set. Thus a press immediately
before the lock event cannot launch after the compositor has locked. In the
response to the
