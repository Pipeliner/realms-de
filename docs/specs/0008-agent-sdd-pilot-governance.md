...
| A3 | A forbidden raw-output-like field, environment-like assignment, credential-like value, source delimiter/snapshot, disallowed command syntax or unknown file/field is supplied | Hygiene/evidence validation fails without echoing it | `hygiene_rejects_invalid_values_in_every_string_class_without_echoing_them`; `hygiene_applies_each_exact_detector_pattern_to_every_string_value`; `malformed_document_prefixed_criteria_must_satisfy_the_prose_grammar` |
| A4 | Evidence or checkpoint not at the record-carrier parent, a root/merge/non-regular carrier, a carrier diff that touches another path or changes an outside-path mode, a non-ancestor workspace base, or dirty/index/untracked workspace is assessed | Exit 2 with the exact `fresh_evidence`, `fresh_checkpoint`, `transition_evidence` and/or `clean_workspace` obligation vector | `root_commit_has_no_freshness_or_transition_evidence`; `merge_commit_has_no_freshness_or_transition_evidence`; `stale_checkpoint_and_nonancestor_base_have_exact_freshness_vectors`; `index_tracked_and_declared_dirty_states_each_make_workspace_unmet_without_writes`; `empty_evidence_reaches_transition_obligations_as_unmet` |
