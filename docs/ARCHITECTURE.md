...
| **Compositor** | River `>= 0.4.0`, with a source selected per target | Fedora 44 has an official native candidate; Ubuntu uses ADR 0022's private pinned closure; Nix uses its locked nixpkgs input. Availability or a source build alone is not graphical-login evidence |
| **Fedora 44 (pre-alpha)** | exactly one pinned Cargo-smoke lane plus exactly one retained-source RPM-build/installroot lane | The exact RPM clean-installs with normal DNF resolution and its installed CLI/palette and River probes pass; graphical login, portals, hardware and SELinux remain unverified |
