...
> reasonably rejected it. Under the new protocol `realm-session` *is* the window
...
`niri` as the interim. The MVP cut line is "one person can use realm as their
...
without owning a compositor: `realm-core` is already roughly 2,900 lines of
source and tests with no Wayland dependency at all.

Building the compositor first means nobody, including us, uses realm for a year.

## Decision

1. `realm-session` talks to a `WmBackend` trait. It never calls a compositor
   directly.
...
3. `NativeBackend` implements the same trait in-process against
   `realm-compositor` from M5.
4. Everything else — clients, themes, keymap, ledger — is written against the
   trait. No file outside `crates/realm-session/src/backend/niri.rs` may mention
   niri.
5. niri stays a *stopgap behind a seam*, per standing order S8.

## niri concept mapping

niri's window model is a scrollable strip of columns per workspace. realm's model
...
| realm concept | niri concept | Fidelity |
|---|---|---|
| Orbit (6, fixed, runes ᚠᚢᚦᚨᚱᚲ) | Workspace | Good, with a caveat: niri workspaces are dynamic and it creates/removes them as they empty. realm pins six named workspaces at startup and refuses to let the count drift |
...
| `focus_step(Next/Prev)` | `focus-column-right` / `focus-column-left` | Good, except wrap-around: realm wraps at the ends, niri stops. Backend must detect the end and jump |
...
| Mono | A single full-width column, or niri's own fullscreen | Approximate. realm's mono keeps the bar's reserved strip; niri fullscreen does not |
...
The honest summary: on niri, realm gets orbits, ordering, focus, swap, fullscreen
...
- Someone can log into realm at M3 rather than M5.
...
- The reference triptych geometry from `Desktop v3.dc.html` is not achievable on
  niri. Early users see an approximation and may reasonably judge realm on it.
- Two window models running at once means state divergence is possible: if niri
  ...
  for existence, while realm remains authority for order.
- niri animates. realm's whole aesthetic is that nothing moves (ADR 0009). We
...
Low, by design. Swapping the backend touches `crates/realm-session/src/backend/`
and the packaging dependency list. Nothing in `realm-core`, `realm-bar`,
`realm-theme` or `realm-ctl` changes. Estimated cost of adding a third backend
...
the triptych approximation is why they stopped using realm, which would argue for
pulling `realm-compositor` forward.

## Guard

- *Planned (M2):* `realm-session` integration test that drives `NiriBackend`
...
- *Planned (M2):* a compile-time check — a CI grep asserting that the string
  ...
  `crates/realm-session/src/backend/`, `packaging/` and `docs/`. This is the
...
1. **Ship the approximation and say so.** `NiriBackend` maps triptych onto the
   ...
   that exact tiling arrives with `realm-compositor`. Cheapest; risks first
   impressions.
...
table only if `realm-compositor` slips past M5.
