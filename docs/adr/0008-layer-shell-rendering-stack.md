...
realm has three layer-shell surfaces: the 32px top bar, the 26px which-key strip
...
- **Rust-first**, and the workspace sets `#![forbid(unsafe_code)]` in
  `realm-core`. We would like clients to stay close to that.
...
Layer-shell clients (`realm-bar`, `realm-hecate`) use:
...
- Buffers are `wl_shm`, allocated at the output's real scale, redrawn only on
  state change. No EGL, no GPU context, no shaders in any realm client.
...
- Damage is tracked: `wl_surface::damage_buffer` is given only the regions that
  changed, and `RealmState::renders_same_as` drops frames that would be identical.
...
- No GPU context anywhere in realm's own clients, so cold start stays inside 900
...
- We hand-roll layout, hit testing and text input. `realm-hecate` needs a text
...
- [ADR 0013](0013-river-window-management-backend.md) does not change this
  stack: `realm-bar` and `realm-hecate` remain ordinary `wlr-layer-shell` clients.
  ...
  `river-layer-shell-v1`, so `realm-session` must serve it before the bar can
...
- `realm-odin` is a `ratatui` TUI in a terminal and is unaffected by this ADR.

## Reversal

Medium. The stack is confined to `crates/realm-bar` and `crates/realm-hecate`, but
it is most of what those crates *are*: a swap is a rewrite of their drawing and
event layers, roughly one to two weeks each. Nothing in `realm-core`,
`realm-session` or `realm-theme` would change, because clients hold no state
...
- *Planned (M2):* a redraw benchmark asserting a full bar repaint stays under
  8 ms, run against the same `RealmState` fixtures the unit tests use.
...
- *Planned (M2):* a link-time assertion that no realm client binary links EGL,
