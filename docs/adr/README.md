...
| [0013](0013-river-window-management-backend.md) | realm **is** the window manager, on river's protocol | Accepted — supersedes [0002](0002-borrow-a-compositor-first.md); Decision 4 superseded for Fedora by [0015](0015-fedora-44-pre-alpha-baseline.md) and Ubuntu by [0022](0022-ubuntu-river-private-closure.md) | river 0.4 moved window management out of the compositor; every row 0002 marked lossy becomes faithful | **Medium** — back to niri is an architecture change, not a module swap |
...
| [0022](0022-ubuntu-river-private-closure.md) | Ubuntu Noble builds a private pinned River closure | Accepted | CI builds the complete feature-bearing River 0.4.8 closure offline after digest-checked acquisition; no hosted repository or target drop | Low once Ubuntu supplies a tested compatible package |
...
| [0013](0013-river-window-management-backend.md) | Does Ubuntu 24.04 supply a River release compatible with `river-window-management-v1`? | **Resolved by [0022](0022-ubuntu-river-private-closure.md).** Noble supplies no River package and several dependencies are below the selected source's floors; Ubuntu retains the target through a private CI-built closure specified by SPEC 0026 |
...
| 0022 | *Planned (#98):* `packaging/river/test-closure-manifest.sh` and the Ubuntu `ubuntu-river-closure` job required by SPEC 0026 |
