...
realm is themed across GTK3, GTK4/libadwaita, Qt5/6, a terminal's 16-colour ANSI
...
1. `palette.toml` is the only file in the repository or on a user's disk that
   may contain a colour literal. `realm-core::palette::Palette` is its schema:
...
|---|---|---|
| **Hand-maintained theme files, one per target** | No template engine, no build step; each file can use its format's full expressiveness; easiest to hand to a contributor who knows GTK CSS but not realm | Forty values times eleven files is the drift problem stated as a number. It also makes "change the accent" a multi-file patch, which is exactly the operation the handoff wants to be one line |
| **A runtime theming daemon that pushes colours over IPC** | No files on disk to get stale; instant application; one authoritative in-memory palette | Only works for clients that speak realm's protocol. GTK, Qt, yazi and btop all read files, so we would need the file pipeline anyway *and* a daemon. Strictly more machinery for the same result |
| **Adopt an existing scheme system** (base16, pywal, stylix) | Large ecosystem of ready-made templates; users already understand it; stylix in particular is idiomatic on our reference platform | base16's sixteen slots cannot express realm's six-step text ramp plus six accents plus per-border alphas without lossy mapping, and the handoff's palette is not negotiable at that fidelity. Stylix would also invert control: the palette would live in a NixOS module rather than in a file that works on Ubuntu and Fedora too. We can *export* a base16 mapping from `palette.toml` later; we cannot import from it without losing values |
...
- Per-file atomic replacement prevents torn files. It does not promise a
  cross-file transaction; the future realm-client notification must occur only
...
- `palette::tests::out_of_range_values_are_rejected_at_parse_time` — including
  `metrics.radius != 0`, which is how "realm has no rounded corners" is enforced.
...
- *Planned (M1):* apply tests that distinguish per-file atomicity from a
  cross-file transaction. The realm-client notification is deferred until its
