...
| 2 | Tile windows: summon, banish, focus, swap, six orbits, triptych + mono | This *is* the window manager | `helm-core` + `helm-session` |
...
| 12 | Install on NixOS, Ubuntu and Fedora | The stated targets | `packaging/` |
...
|---|---|---|
| `helm-compositor` (Smithay) | niri via `NiriBackend` | M5 |
...
| **M2** | Session and bar | `helm-session` + `NiriBackend`, `helm-bar` | Bar reflects live orbit/focus/mode changes on niri |
...
2. **Stopgaps must be swappable.** Every stopgap (fuzzel, niri, btop) sits
   behind a config or a trait, never a hardcoded call.
