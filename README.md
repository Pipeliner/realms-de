...
| Package/session/portal assets | **Tracked pre-alpha contract with bounded installed evidence on NixOS, Ubuntu and Fedora.** The installed NixOS QEMU VM produces the captured desktop and verifies a FileChooser cancel, Settings read, and nonempty ScreenCast buffer; exact Ubuntu Realm and private River packages clean-install together in an empty amd64 Noble root; the exact Fedora RPM verified in an empty installroot runs installed CLI, palette and River probes. Native Ubuntu and Fedora graphical login, browser picker and physical hardware remain pending. |
