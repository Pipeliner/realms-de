...
**2026-08-26 — `realm-core` written before anything else, and it owns every
...
**2026-08-26 — ~~Ship on niri first~~ — superseded the same day. realm is instead
...
realm's ledger model rather than an approximation of it. The niri route would
...
channels rotates its hue; realm's violet drifted 18° toward blue at contrast 1.4
...
**2026-08-26 — The systemd unit for the window manager is `realm-wm.service`,
matching the binary.** *(Briefly instructed as `realm-sessiond.service` before
this was settled; caught by the packaging agent noticing the unit then matched
no binary that existed.)* Reason: — because three names for one thing (`realm-daemon`,
`realm-sessiond`, `realm-session`) had already produced a collision once, and a
...
**2026-08-26 — Theme rendering happens in `realmctl`, never in `realm-session`** —
...
its official repositories exposed River 0.4.8, the protocol generation Realm
requires. This is package-availability evidence, not a tested Realm/River
