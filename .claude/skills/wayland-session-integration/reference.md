...
none of realm's own session files exist yet, so the commands below are how you
...
```
              ...
              graphical-session.target        ← the compositor reaches this
                 ...
                 │        │        │             imported
        realm-bar.service  │   xdg-desktop-portal.service
                          │
                 realm-idle.service (lock, dpms)
```

Rules:

- Every realm client unit: `PartOf=graphical-session.target` (so it stops with
...
- `XDG_CURRENT_DESKTOP=realm` is what `portals.conf` matches on. It must be set
  before `xdg-desktop-portal` starts, which in practice means before anything
  can D-Bus-activate it.
- Ship a `realm-portals.conf` naming a concrete backend per interface rather than
  ...
  anywhere.
- realm's own file dialog (`charon portal`, via
...
2. `XDG_CURRENT_DESKTOP=realm` and `XDG_SESSION_TYPE=wayland`, likewise.
3. A portal backend answers, and it implements FileChooser and ScreenCast.
4. Cursor theme resolvable and set in gsettings.
5. Font stack covers the glyph inventory — `realm-core::glyphs::Probe::summary()`
   already produces exactly this line.
6. Session protocol version matches `realm-core::ipc::PROTOCOL_VERSION`.
