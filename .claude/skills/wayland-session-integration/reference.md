...
| Nothing themed after login, though `helm ctl theme apply` works by hand | user units started before the import, so they inherited an empty environment | `systemctl --user show-environment` compared against the compositor's own `/proc/<pid>/environ` |
