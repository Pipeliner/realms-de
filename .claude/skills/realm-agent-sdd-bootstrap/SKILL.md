---
name: realm-agent-sdd-bootstrap
...
`realm-spec-first` and `realm-issue-flow` for those tasks.

## Orient one issue

1. Read `AGENTS.md`, `CLAUDE.md`, `.claude/memory/00-standing-orders.md` and
   `.claude/memory/40-loop.md`.
2. Follow `realm-issue-flow` to select and track the GitHub issue. Follow
   `realm-spec-first` before any non-trivial implementation work.
...
5. Use the existing read-only assessment only after the record is fresh and the
   ...
   ```text
   realm-sdd gate --issue <issue> --from <maturity> --to <maturity>
   realm-sdd promote --dry-run --issue <issue> --from <maturity> --to <maturity>
   ```

`realm-sdd` currently exposes only `gate` and `promote --dry-run` for this
