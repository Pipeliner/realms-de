---
name: realm-perf-budget
description: Use before adding a timer, a poll loop, a `sleep`, an animation or transition, a redraw or damage path, a background thread, or any work in a hot path such as layout projection, bar rendering or theme apply. Also use when asked "is this fast enough", "why is idle CPU not zero", "can we animate this", "can we add a blur or rounded corners", or when touching frame budgets, RealmState::renders_same_as, damage tracking, or anything under ARCHITECTURE.md §4.
---

# realm performance budget
...
| State change → bar redraw | **< 8 ms** | damage tracking; `RealmState::renders_same_as` drops no-op frames |
...
| `realm ctl theme apply` | **< 150 ms** | one captured input set is rendered, validated, sealed, made durable, and selected as an immutable generation for future launches; no pointer-switch reload |
...
served by the one shared 1 Hz sampler in `realm-session`: CPU, memory, GPU and
...
`RealmState::renders_same_as` compares everything that is visible and ignores
...
There is no benchmark harness in the workspace yet — `realm-core` has no
...
```
cargo test --release -p realm-core         # release timings, not debug
