...
| State change → bar redraw < 8 ms | The bar lags behind the thing it is describing | `RealmState` is a plain data struct; `renders_same_as` gates the frame before any drawing |
...
2. **Can the producer be asked to notify?** A realm-owned component pushing is
...
Run with `cargo test --release -p realm-core -- --nocapture`. `black_box` matters:
...
```sh
pidstat -p "$(pgrep -x realm-bar)" 1 60    # a minute of genuine idleness
```
...
```sh
systemd-analyze --user critical-chain realm-session.target
