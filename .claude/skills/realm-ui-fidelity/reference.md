# realm-ui-fidelity reference

Token tables and per-surface anatomy. Every value here is a **field in
`palette.toml`**, parsed into `realm_core::Palette` — read it from there rather
...
`realm_core::glyphs::inventory()` is the complete list; `Surface` tags where each
...
Left to right: `✦ realm` logo in violet with a 1px right border; six orbit runes
...
The bar's data is `RealmState`: `orbits` (six `OrbitCell`s with `OrbitDisplay`),
