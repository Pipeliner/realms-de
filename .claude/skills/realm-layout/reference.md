# realm-layout reference

Exact contracts for `crates/realm-core/src/layout.rs` and
`crates/realm-core/src/ledger.rs`. Open this when you need a signature or are
...
| `WinId(pub u64)` | Compositor handle. Opaque; realm never derives meaning from the number. |
...
Run one with
`cargo test -p realm-core layout::tests::<name>` (or `ledger::tests::<name>`).
