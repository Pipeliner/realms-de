...
The governing spec is `docs/specs/0001-realm-core-contracts.md`, whose acceptance
...
cargo test -p realm-core layout::tests::every_layout_tiles_exactly_for_every_plausible_size
cargo test -p realm-core layout::tests::projection_is_pure_and_focus_only_moves_the_flag
cargo test -p realm-core layout::tests::partition_is_exact_and_stable
cargo test -p realm-core layout::tests::absurd_params_are_clamped_rather_than_producing_negative_rects
cargo test -p realm-core                      # the whole crate, last
```
...
- **Serde.** `Layout` is `#[serde(rename_all = "kebab-case")]` and crosses the
  IPC boundary inside `RealmState`. A new variant is a protocol change: bump
