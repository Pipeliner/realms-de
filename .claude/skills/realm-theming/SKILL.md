...
shipped palette is `include_str!`-ed into `realm-core`'s tests, so a bad edit
...
`docs/ARCHITECTURE.md` §4), and it rotates hues — realm's violet drifted about
18° toward blue at contrast 1.4 before the current implementation.

How it actually works, in `crates/realm-core/src/color.rs`:
...
5. Verify with `cargo test -p realm-core palette` and `cargo test -p realm-theme`
...
```
$EDITOR palette.toml
cargo test -p realm-core palette::tests::shipped_palette_passes_its_own_lint
cargo test -p realm-core palette::tests::shipped_palette_survives_the_whole_contrast_range
cargo test -p realm-core                     # the whole crate
```
...
`reference.md` holds the reach-and-limits table (what realm's theme can and
