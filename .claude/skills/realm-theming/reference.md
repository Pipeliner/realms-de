# realm-theming reference

Open this when theming a new app, or when you need an exact signature from
`crates/realm-core/src/color.rs` or `palette.rs`. The rules and the decision
...
| CSD headerbars | client-drawn by definition | realm recolours them and adds its 1px border; the shape stays theirs |
...
`crates/realm-core/src/palette.rs`.
...
`crates/realm-core/src/color.rs`.
...
Run with `cargo test -p realm-core <name>`.
